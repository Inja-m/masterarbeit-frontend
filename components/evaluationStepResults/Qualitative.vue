<template>
  <h2>User Stories</h2>
  <p class="text-sm">
    Die hier dargestellten User Stories wurden im Rahmen einer qualitativen
    Auswertung erstellt. Ihr <strong>Status</strong> wird
    <strong>unabhängig</strong> von dieser Auswertung gepflegt und durch die
    Enticklung aktualisiert. <br /><br />
    Das bedeutet: Auch wenn eine User Story bereits im Rahmen der Auswertung
    dokumentiert wurde, kann ihr Status später, basierend auf dem
    Entwicklungsfortschritt, angepasst werden.
  </p>
  <UCarousel
    v-slot="{ item }"
    :items="result"
  ...
  >
    <UCard class="text-sm" :class="[getColor(item.developmentStatus)]">
      ...
      >
        ...
        <div v-if="item.result.Link?.length">
          <UButton
